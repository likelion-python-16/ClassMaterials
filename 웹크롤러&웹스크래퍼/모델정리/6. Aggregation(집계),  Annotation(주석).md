...
| 항목         | `Aggregation`        | `Annotation`                 |
| ---------- | -------------------- | ---------------------------- |
| 의미         | 전체 쿼리셋에 대한 집계 결과를 반환 | 각 객체에 집계 결과를 필드처럼 추가         |
| 결과 형태      | **단일 값** (dict 형태)   | **쿼리셋** (각 객체에 필드가 추가됨)      |
| 대표 예시      | 전체 평균, 총합, 개수        | 각 유저의 게시글 수, 각 카테고리의 리뷰 평균 등 |
| 사용 위치    . | `.aggregate()` 메서드   | `.annotate()` 메서드            |
...
- StdDev: 표준편차
- Variance: 분산
`StdDev`(표준편차), `Variance`(분산) 같은 통계적 집계 함수도 Django ORM에서 계산 후 시각화에 활용할 수 있습니다.
- `Variance` (분산): 각 데이터가 평균에서 얼마나 떨어져 있는지의 제곱합 평균
- `StdDev` (표준편차): 분산의 제곱근 → 데이터의 실제 흩어짐 정도
