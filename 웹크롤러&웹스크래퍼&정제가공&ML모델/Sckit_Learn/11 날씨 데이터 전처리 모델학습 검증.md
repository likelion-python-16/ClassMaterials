...
1. 카테고리별 분포 패턴
    - 예를 들어, “기온이 높고 강수량이 낮을 때 주로 어떤 음식(카페·초밥 등)이 선택되는지” 같은 클러스터(군집)가 보이는지 확인
    - 서로 다른 음식 카테고리가 피처 공간에서 겹치는지, 분리되는지 시각적으로 파악
        
2. 피처 간 관계 탐색
    - 온도–강수량, 온도–흐림도 같은 2개 피처 간 연관성을 살펴보고,
    - 앞서 본 상관계수 히트맵과 달리 “카테고리별로 값이 어떻게 분포하는지”를 동시에 볼 수 있음
        
3. 이상치·데이터 범위 확인
    - 이상치 제거 후에도 남은 점들이 어디쯤 몰려 있는지 확인
    - 특정 카테고리에만 극단적인 값(예: 매우 높은 강수량)이 있는지 탐색
---
정리  
> 이 산점도는 “피처 간 선형 상관계수” 를 보여주기 위한 게 아니라,  
> 여러 기상 피처들의 쌍(pairwise) 분포 위에서  
> Category(음식 종류)가 어떻게 뭉치고 흩어져 있는지를 한눈에 확인하기 위한 시각화입니다.
